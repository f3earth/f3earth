...
export class TileMesh {
    ...
    _calcMercatorBound() {
        ...
        let metersPerPixel = this._getMetersPerPixel(this._zoom);
    ...
    _calcLatLng(x, y, isFirstRow, isLastRow) {
        ...
        let latLng = Proj.mercator2Wgs84(x, y);
    ...
    _createMesh() {
        ...
        let intervalMeters = Math.abs(this._bound.E - this._bound.W) / SEGMENT_COUNT;
        let maxRow = (1 << this._zoom) - 1;
        ...
        for (let y = 0; y <= SEGMENT_COUNT; y++) {
            ...
            for (let x = 0; x <= SEGMENT_COUNT; x++) {
                ...
                let latLng = this._calcLatLng(pointW, pointN, this._row === 0 && y === 0, this._row === maxRow && y === SEGMENT_COUNT);

                let pointX = this._radius * Math.sin(latLng.lng * Math.PI / 180) * Math.cos(latLng.lat * Math.PI / 180);
                let pointY = this._radius * Math.sin(latLng.lat * Math.PI / 180);
                let pointZ = this._radius * Math.cos(latLng.lng * Math.PI / 180) * Math.cos(latLng.lat * Math.PI / 180);
